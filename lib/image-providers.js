...
// Converts any PNG to clean black-outlines-on-white line art via Sobel edge
// detection. Called when a provider's quality check fails — guarantees coloring-
// book style output even from photorealistic FLUX.1-schnell renders.
// Pure Node.js (no external deps). Returns null on any failure.
...
function convertToLineArt(buf) {
  ...
  // Reconstruct filtered scanlines + convert to grayscale
  ...
  // Gaussian 3×3 blur — reduces noise before edge detection
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      for (let ky = -1; ky <= 1; ky++) for (let kx = -1; kx <= 1; kx++) {
        ...
        sum += gray[ny * w + nx] * K[(ky + 1) * 3 + (kx + 1)];
      }
      blurred[y * w + x] = (sum + 8) >> 4;
    }
  }
  ...
  // Adaptive threshold: 18% of peak gradient captures main outlines without noise
  const THRESH = maxMag * 0.18;
  const edge = new Uint8Array(w * h);
  for (let i = 0; i < w * h; i++) if (magnitudes[i] >= THRESH) edge[i] = 1;

  // Dilate by 1 pixel to produce bold, kid-friendly line weight
  const output = Buffer.alloc(w * h, 255); // start all white
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      if (edge[y * w + x]) {
...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  if (!skipToFreeTiers) {
    try {
      ...
      // Quality check failed — try line-art post-processor before discarding
      const lineArt = convertToLineArt(result.buffer);
      if (lineArt) {
        console.log("[providers] Pollinations post-processed to coloring-book line art");
        return { buffer: lineArt, contentType: "image/png" };
      }
      console.warn("[providers] Pollinations dark image — post-processing also failed, trying next tier");
    } catch (err) {
  ...
  // Tier 4: Novita.ai (paid) — last resort; post-process if needed, never block.
  if (novitaKey) {
    try {
      ...
      // Last paid resort — post-process first, then send as-is rather than blocking
      const lineArt = convertToLineArt(result.buffer);
      if (lineArt) {
