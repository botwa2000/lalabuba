...
// Compress-size is a cheap no-dependency quality proxy:
//   PNG 1024×1024 coloring page (mostly white):  50–300 KB
//   PNG 1024×1024 dark/shaded image:             380 KB+
//   JPEG 1024×1024 coloring page:                15–120 KB
//   JPEG 1024×1024 dark/shaded image:            200 KB+
// Thresholds are deliberately loose to avoid false positives on detailed pages.
...
function checkColoringPageQuality(buffer, contentType) {
  ...
  if (isPng) {
    const THRESHOLD_KB = 380;
    if (kb > THRESHOLD_KB) {
      return { ok: false, reason: `PNG too large (${kb} KB > ${THRESHOLD_KB} KB — likely dark/shaded)`, kb };
    }
  ...
  if (isJpeg) {
    ...
    if (kb > THRESHOLD_KB) {
      return { ok: false, reason: `JPEG too large (${kb} KB > ${THRESHOLD_KB} KB — likely dark/shaded)`, kb };
    }
