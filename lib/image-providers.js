...
async function generateWithTogetherAI(prompt, width, height, seed, apiKey) {
  ...
  if (!res.ok) {
    // Quota/busy OR auth/permission failure → skip this tier (don't abort the
    // whole waterfall; a misconfigured key shouldn't 500 the request).
    if ([401, 402, 403, 429].includes(res.status) || res.status >= 500) throw new Error("__QUOTA__");
