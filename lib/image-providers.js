...
async function generateWithPollinations(prompt, width, height, seed) {
  ...
  if (!upstream.ok) {
    if (upstream.status === 429) throw new Error("__QUOTA__");
  ...
  if (!contentType.startsWith("image/")) {
    // Pollinations returned JSON/text (NSFW flag, error, etc.) — fall through to next provider
...
async function generateWithTogetherAI(prompt, width, height, seed, apiKey) {
  ...
  if (!res.ok) {
    if (res.status === 429 || res.status === 402) throw new Error("__QUOTA__");
