...
function _edgeArtFromGray(gray, w, h) {
  ...
  // Dilate by 2 pixels (5×5 neighbourhood) for two reasons:
  // 1. Bold kid-friendly lines that are easy to see and color within.
  // 2. Gap-closing: small outline gaps (≤4px) are bridged, so BFS region
  //    detection finds enclosed areas. Tested with 3×3 (±1px) — resulted in
  //    only 1 enclosed region for complex subjects because gaps in the Sobel
  //    output broke contour continuity. Restored to 5×5 which reliably closes
  //    those gaps and produces 5-12 numbered regions per image.
  ...
  // Fragmented images (scattered texture noise from the AI) produce 50-200+ components
