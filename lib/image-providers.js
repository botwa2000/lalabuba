...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  if (novitaKey) {
    try {
      // Adaptive timeout: give Novita whatever budget remains under the 120s Flutter limit,
      // keeping 10s for HuggingFace fallback. Floor at 30s so a very slow Pollinations race
      // doesn't leave Novita with too little time; cap at NOVITA_TIMEOUT_MS() (80s).
      const elapsed = Date.now() - t0;
      const CLIENT_BUDGET_MS = 110_000; // 10s margin under 120s Flutter client timeout
      const novitaMs = Math.min(NOVITA_TIMEOUT_MS(), Math.max(30_000, CLIENT_BUDGET_MS - elapsed - 10_000));
