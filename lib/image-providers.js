...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  // Last-resort fallback: if Pollinations produces a dark-but-valid image and all
  // other providers also fail, serve it rather than throwing "service busy".
  let pollinationsFallback = null;
  ...
  if (!skipToFreeTiers) {
    try {
      ...
      // Dark image — save as fallback instead of discarding completely
      pollinationsFallback = result;
      console.warn("Pollinations dark image saved as fallback — trying next tier");
    } catch (err) {
  ...
  // Last resort: serve the Pollinations dark image rather than a 500 error.
  // The image may be photorealistic but at least users get something to color.
  if (pollinationsFallback) {
    console.warn("[providers] all tiers failed — using Pollinations dark-image fallback");
