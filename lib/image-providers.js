...
function buildPrompt(subject, difficulty = "medium", size = "medium") {
  const base = [
    ...
    "no open line ends",
    // Monochrome — must be pure uncolored line art (a coloured subject like a
    // rainbow must still be drawn as empty outlined bands, NOT filled in).
...
// Pollinations (free tier 1) is fast on a cache HIT (~1-2s) but a cache MISS
// (every novel subject+seed) is now useless: the anonymous tier either returns
// HTTP 429 instantly OR accepts the connection and HANGS in a generation queue
// until the timeout. So the timeout is pure dead weight on every cold miss —
// time spent before the waterfall can reach the tier that will actually serve
// the image (currently paid Novita, ~6s warm / ~17s cold-start). Measured: a
// cache HIT resolves in ~1-3s, so 6s keeps the fast+free hit path while cutting
// ~6s of wasted hang off every novel subject. Tune via env.
const POLLINATIONS_TIMEOUT_MS = (() => {
  const v = Number(process.env.POLLINATIONS_TIMEOUT_MS);
  return Number.isFinite(v) && v > 0 ? Math.floor(v) : 6_000;
})();
...
async function generateWithTogetherAI(prompt, width, height, seed, apiKey) {
  ...
  if (!res.ok) {
    // Any non-OK → skip this tier; the waterfall falls through to the next
    // provider instead of aborting the whole request.
  ...
  if (!isValidImageBuffer(buf)) throw new Error("__QUOTA__"); // bad bytes → try next tier
...
async function generateWithCloudflare(prompt, width, height, seed, accountId, apiToken) {
  ...
  if (!isValidImageBuffer(cfBuf)) throw new Error("__QUOTA__"); // bad bytes → try next tier
  return {
...
async function generateWithNovita(prompt, width, height, seed, apiKey) {
  ...
  const imgRes = await fetch(safeUrl, { signal: AbortSignal.timeout(FETCH_TIMEOUT_MS) });
