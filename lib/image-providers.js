...
const COLORING_NEGATIVE_PROMPT =
  ...
  "circular frame, circular border, medallion, decorative border, ornamental frame, " +
  "circle composition, badge, emblem, seal, coin design, round frame, oval frame";

function buildStructuredPrompt(subject, difficulty, size) {
  const base = [
    // Composition — full subject must be visible in frame, no circular borders.
    "full body view",
  ...
  return [
    ...
    "bold structural divider lines separating body parts limbs and features into distinct enclosed areas",
    "no hatching no crosshatching no stippling no texture lines",
...
function _edgeArtFromGray(gray, w, h) {
  ...
  // Double-pass Gaussian 7×7 blur (σ≈2, applied twice → effective σ≈2.83).
  // Double-pass suppresses photorealistic AI texture/shading noise far more
  // aggressively than a single pass, so only dominant structural edges survive
  // into the Sobel stage. This function is only called on dark/shaded images
  // that failed the quality check, so heavy blur is correct.
    ...
    for (let y = 0; y < h; y++) {
      for (let x = 0; x < w; x++) {
        ...
        for (let ky = -3; ky <= 3; ky++) for (let kx = -3; kx <= 3; kx++) {
          ...
          sum += src[ny * w + nx] * K7[(ky + 3) * 7 + (kx + 3)];
        }
        dst[y * w + x] = Math.round(sum / K7SUM);
      }
    }
    return dst;
  }
  const blurred = _applyK7(_applyK7(gray, w, h), w, h);
  ...
  // Adaptive threshold: 50% of peak gradient — captures only dominant structural
  // edges (body outline, major limbs, props). Double-pass blur already removed
  // texture/shading noise, so 50% is safe and produces clean connected outlines.
  // 40% was too low after single-pass blur: it still captured shading gradients
  // and produced 50+ disconnected fragments that could not form closed regions.
  const THRESH = maxMag * 0.50;
  const edge = new Uint8Array(w * h);
  for (let i = 0; i < w * h; i++) if (magnitudes[i] >= THRESH) edge[i] = 1;

  // Dilate by 2 pixels (5×5 neighbourhood):
  // 1. Bold kid-friendly lines easy to see and color within.
  // 2. Gap-closing: bridges outline gaps up to 4px so BFS region detection
  //    finds enclosed areas. 5×5 gives clean closed regions on typical subjects.
  ...
  // Connectivity filter: remove isolated noise blobs too small to be a real stroke.
  // MIN_BLOB_PX=150: removes source fragments ≤26px long after 5×5 dilation.
  // Feather dashes / scale marks (3–20px source) → 35–120px after dilation → removed.
  // Real structural lines (30px+) → 170px+ after dilation → kept.
  const MIN_BLOB_PX = 150;
  let survivingBlobCount = 0;
  const survivingSizes = [];
  ...
  for (let start = 0; start < w * h; start++) {
    ...
    if (component.length < MIN_BLOB_PX) {
      ...
      survivingBlobCount++;
      survivingSizes.push(component.length);
    }
  }

  // Dominant-blob check: if one component holds >70% of all surviving black pixels
  // and there are fewer than 3 other meaningful components (≥500px each), the image
  // is a plain silhouette or circular blob with no internal structure — BFS flood-fill
  // cannot find colorable sub-regions inside it. Reject so the waterfall tries a tier
  // that generates an image with actual interior structural lines.
  if (survivingSizes.length >= 1) {
    const totalSurviving = survivingSizes.reduce((a, b) => a + b, 0);
    const maxSize = Math.max(...survivingSizes);
    const dominanceRatio = maxSize / Math.max(1, totalSurviving);
    const otherMeaningful = survivingSizes.filter(s => s >= 500 && s !== maxSize).length;
    if (dominanceRatio > 0.70 && otherMeaningful < 3) {
      console.warn(`[quality] Dominant blob (${(dominanceRatio * 100).toFixed(0)}% of black, ${otherMeaningful} other significant components) — silhouette with no interior structure, rejecting`);
      return null;
    }
  }

  // Fragment check: too many disconnected black components = noisy/fragmented art.
  // Double-pass blur + 50% threshold should produce 5–30 components on typical
  // subjects. Cap at 50 to catch genuinely pathological noise while allowing
  // complex scenes with many body parts, props, and background elements.
  const MAX_LINE_FRAGMENTS = 50;
  ...
  // Minimum floor: a pure silhouette (outer contour only, no internal lines) produces
  // 1–4 components after dilation. With no internal structural lines, flood-fill finds
  // at most 1 numbered region even if the outline is perfectly closed. Require ≥5 so
  // we only accept images with enough internal detail to form multiple colorable regions.
  const MIN_LINE_FRAGMENTS = 5;
  if (survivingBlobCount < MIN_LINE_FRAGMENTS) {
    console.warn(`[quality] Too few line components (${survivingBlobCount} < ${MIN_LINE_FRAGMENTS}) — image is a simple silhouette with no interior lines, rejecting`);
    return null;
  }
  ...
  // Require at least 1.5% black pixels — below that there is not enough outline
  // for a coloring page. 3% was too strict and rejected valid sparse-but-clean images.
  let blackCount = 0;
...
// 115s: raised from 80s because Novita consistently needs 80-115s; Flutter timeout is now 150s
// so there is budget. Worst case: 45s (Pollinations) + 85s (adaptive cap) + 0s (HF 401) = 130s < 150s.
const NOVITA_TIMEOUT_MS       = () => (Number.isFinite(_envNovita) && _envNovita > 0) ? _envNovita : (dc().providers?.novitaTimeoutMs       ?? 115_000);
...
async function generateWithCloudflare(prompt, width, height, seed, accountId, apiToken) {
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ prompt, steps: dc().providers?.steps?.cloudflare ?? 8 }),
    });
    if (!res.ok) {
      console.warn(`Cloudflare AI ${res.status} — skipping tier`);
      throw new Error("__QUOTA__");
    }
    // CF Workers AI returns JSON: {result:{image:"<base64-png>"}}
    const json = await res.json();
    const b64 = json?.result?.image;
    if (!b64) {
      console.warn("Cloudflare AI: no image in response:", JSON.stringify(json).slice(0, 120));
      throw new Error("__QUOTA__");
    }
    const cfBuf = Buffer.from(b64, "base64");
    if (!isValidImageBuffer(cfBuf)) throw new Error("__QUOTA__");
    return { contentType: "image/png", buffer: cfBuf };
  } finally {
    ctrl.cancel();
  }
}
...
// Circuit breaker: Novita's generate call should return in ~8-20s. Diagnosed
// 2026-09-19 (see project-image-generation-providers memory): with a valid,
// funded account, it instead HANGS to the full adaptive timeout on every
// request (confirmed via direct probe — an unauthenticated call 403s in
// ~500ms, but an authenticated real-generation call never returns). Burning
// the full ~115s budget on a tier that is currently dead pushes total
// generation time past the Flutter client's own timeout, so users see "took
// too long" even though the waterfall would have reached a real image via
// HuggingFace/last-resort. After NOVITA_BREAKER_THRESHOLD consecutive
// failures, "open" the circuit: give Novita a short probe timeout instead of
// the full one, so a dead tier fails fast. Any success (even via the short
// probe) closes the circuit again — this self-heals once Novita recovers.
const _envBreakerThreshold = Number(process.env.NOVITA_BREAKER_THRESHOLD);
const _envBreakerProbeMs = Number(process.env.NOVITA_BREAKER_PROBE_MS);
const NOVITA_BREAKER_THRESHOLD = (Number.isFinite(_envBreakerThreshold) && _envBreakerThreshold > 0) ? _envBreakerThreshold : 2;
const NOVITA_BREAKER_PROBE_MS = (Number.isFinite(_envBreakerProbeMs) && _envBreakerProbeMs > 0) ? _envBreakerProbeMs : 10_000;
let _novitaConsecutiveFailures = 0;

function novitaCircuitOpen() {
  return _novitaConsecutiveFailures >= NOVITA_BREAKER_THRESHOLD;
}

async function generateWithNovita(prompt, width, height, seed, apiKey, timeoutMs) {
  // Daily-cap refusals are an intentional spend guard, not evidence Novita
  // itself is unhealthy — check it here, outside the breaker's try/catch, so
  // hitting the cap doesn't get counted as a "failure" toward opening the circuit.
  if (novitaCapReached()) {
  ...
  const breakerOpen = novitaCircuitOpen();
  const effectiveTimeoutMs = breakerOpen ? NOVITA_BREAKER_PROBE_MS : timeoutMs;
  if (breakerOpen) {
    console.warn(`[providers] Novita circuit OPEN (${_novitaConsecutiveFailures} consecutive failures) — probing with ${NOVITA_BREAKER_PROBE_MS}ms instead of the full timeout`);
  }
  try {
    const out = await _generateWithNovitaInner(prompt, width, height, seed, apiKey, effectiveTimeoutMs);
    _novitaConsecutiveFailures = 0;
    return out;
  } catch (err) {
    _novitaConsecutiveFailures++;
    throw err;
  }
}

async function _generateWithNovitaInner(prompt, width, height, seed, apiKey, timeoutMs) {
...
// ─── Legacy: HuggingFace (routed via the Nscale inference provider) ─────────

// The router previously targeted "nebius/models/{model}" (the old direct
// hf-inference-style path). Nebius has never served FLUX.1-schnell (confirmed
// via https://huggingface.co/api/models/black-forest-labs/FLUX.1-schnell
// ?expand[]=inferenceProviderMapping — only nscale/fal-ai/wavespeed are live),
// so every call 404'd unconditionally — this tier never actually worked as a
// fallback. Nscale DOES serve this model, but through an OpenAI-compatible
// /v1/images/generations endpoint (b64_json body), not the /models/{id} path —
// confirmed 2026-09-20 via a live authenticated call (200 OK, ~4s).
async function generateWithHuggingFace(prompt, width, height, seed, hfToken, hfModel) {
  ...
  const ctrl = abortAfter(FETCH_TIMEOUT_MS());
  try {
    const upstream = await fetch("https://router.huggingface.co/nscale/v1/images/generations", {
      method: "POST",
      signal: ctrl.signal,
      headers: {
    ...
    let b64 = json.data?.[0]?.b64_json;
    if (!b64) throw new Error("Hugging Face returned no image data");
    const comma = b64.indexOf(",");
    if (comma !== -1) b64 = b64.slice(comma + 1);
    const buffer = Buffer.from(b64, "base64");
    if (!isValidImageBuffer(buffer)) throw new Error("Hugging Face returned invalid image bytes");
...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  // Best real (if quality-rejected) candidate seen across the whole waterfall — the
  // absolute last resort if every tier fails outright. See bottom of this function.
  let lastResort = null;
  ...
  // Binarize a clean coloring-page JPEG → pure B&W PNG.
  // High-brightness JPEG images from providers like Cloudflare are already coloring
  // templates, but JPEG compression blurs outline edges (creating gray fringe pixels)
  // and the resulting anti-aliased lines can have micro-gaps that prevent Flutter's
  // flood-fill from finding enclosed regions. Simple threshold at 128 converts edge
  // fringe to solid black (slightly thickening outlines), removes decorative gray
  // texture marks (which never form closed regions anyway), and gives the Flutter
  // client crisp binary lines that reliably close into colorable areas.
  // Note: this is NOT Sobel edge-detection — we threshold the existing coloring-page
  // content, not compute new edges from it. No doubled outlines or edge artifacts.
  async function binarizeColoringPage(buf, name) {
    const sharp = _tryGetSharp();
    ...
    try {
      const result = await sharp(buf)
        ...
        .png()
        .toBuffer();
      console.log(`[quality] ${name} → binarized to pure B&W PNG (threshold 128)`);
      return result;
    } catch {
      return null;
    }
  ...
  // Count enclosed white regions in a binarized B&W PNG.
  // A proper coloring page has many enclosed areas (body, wing, head, etc.)
  // that become numbered regions in Flutter's flood-fill. A silhouette
  // produces only 1-3 tiny white holes in an otherwise solid black body.
  // Regions smaller than MIN_REGION_PX are ignored (JPEG binarization specks).
  async function checkEnclosedRegions(pngBuf, name) {
    const sharp = _tryGetSharp();
    if (!sharp) return true; // can't check without sharp — assume OK
    try {
      const { data, info } = await sharp(pngBuf).greyscale().raw().toBuffer({ resolveWithObject: true });
      const w = info.width, h = info.height, total = w * h;
      const px = data; // Uint8Array: 0=black, 255=white after greyscale

      // Phase 1: BFS from border white pixels to identify the background.
      const isBg = new Uint8Array(total);
      const q = new Int32Array(total);
      let qH = 0, qT = 0;
      const enq = (i) => { if (!isBg[i] && px[i] > 128) { isBg[i] = 1; q[qT++] = i; } };
      for (let x = 0; x < w; x++) { enq(x); enq((h - 1) * w + x); }
      for (let y = 1; y < h - 1; y++) { enq(y * w); enq(y * w + w - 1); }
      while (qH < qT) {
        const p = q[qH++], col = p % w, row = (p - col) / w;
        if (col > 0)     enq(p - 1);
        if (col < w - 1) enq(p + 1);
        if (row > 0)     enq(p - w);
        if (row < h - 1) enq(p + w);
      ...
      // Phase 2: count enclosed white blobs (white pixels not reachable from border).
      const MIN_REGION_PX = Math.max(100, Math.floor(w * h * 0.0003)); // ~0.03% of image
      const seen = new Uint8Array(total);
      const rq = new Int32Array(total);
      let enclosedCount = 0;
      let maxRegionSize = 0;
      for (let start = 0; start < total; start++) {
        if (px[start] <= 128 || isBg[start] || seen[start]) continue;
        let rH = 0, rT = 0, size = 0;
        seen[start] = 1; rq[rT++] = start;
        while (rH < rT) {
          const p = rq[rH++]; size++;
          const col = p % w, row = (p - col) / w;
          if (col > 0     && !seen[p - 1] && !isBg[p - 1] && px[p - 1] > 128) { seen[p - 1] = 1; rq[rT++] = p - 1; }
          if (col < w - 1 && !seen[p + 1] && !isBg[p + 1] && px[p + 1] > 128) { seen[p + 1] = 1; rq[rT++] = p + 1; }
          if (row > 0     && !seen[p - w] && !isBg[p - w] && px[p - w] > 128) { seen[p - w] = 1; rq[rT++] = p - w; }
          if (row < h - 1 && !seen[p + w] && !isBg[p + w] && px[p + w] > 128) { seen[p + w] = 1; rq[rT++] = p + w; }
        }
        if (size >= MIN_REGION_PX) {
          enclosedCount++;
          if (size > maxRegionSize) maxRegionSize = size;
        }
      }

      // Reject medallion/circle-interior style: a single region occupying >25% of
      // the image area means the border creates one huge enclosed interior — not a
      // proper coloring page (no internal subdivisions usable for colour-by-number).
      const MAX_SINGLE_REGION_FRAC = 0.25;
      const maxAllowed = Math.floor(total * MAX_SINGLE_REGION_FRAC);
      if (maxRegionSize > maxAllowed) {
        console.warn(`[quality] ${name} binarized image: largest enclosed region ${maxRegionSize}px = ${(maxRegionSize / total * 100).toFixed(1)}% of image (> ${(MAX_SINGLE_REGION_FRAC * 100).toFixed(0)}%) — medallion/circle interior, rejecting`);
        return false;
      }

      const MIN_ENCLOSED = 5; // require ≥5 enclosed regions (matches MIN_LINE_FRAGMENTS in Sobel path)
      if (enclosedCount < MIN_ENCLOSED) {
        console.warn(`[quality] ${name} binarized image: only ${enclosedCount} enclosed colorable regions (< ${MIN_ENCLOSED}) — silhouette, rejecting`);
        return false;
      }
      console.log(`[quality] ${name} binarized enclosed regions OK: ${enclosedCount}, max region ${(maxRegionSize / total * 100).toFixed(1)}%`);
      return true;
    } catch (e) {
      console.warn(`[quality] checkEnclosedRegions error: ${e.message}`);
    ...
    if (isJpeg) {
      ...
      if (!isNaN(brightness)) {
        ...
        if (brightness < JPEG_BRIGHT_MIN) {
          ...
          if (lineArt) {
            // Apply the same enclosed-region check to Sobel output — circular medallion
            // drawings produce a solid circle border with unclosed interior lines; the
            // Sobel blob/density checks pass but 0 flood-fill regions survive in the app.
            if (!(await checkEnclosedRegions(lineArt, name))) {
              lastResort = { buffer: lineArt, contentType: "image/png" };
              return null;
            }
        ...
        // Binarize clean coloring-page JPEGs before sending to Flutter so the client
        // receives pure B&W with crisp outlines instead of JPEG-compressed edge fringes
        // that cause open-outline gaps in Flutter's flood-fill region detection.
        const binarized = await binarizeColoringPage(buf, name);
        if (binarized) {
          // Reject silhouette images: FLUX sometimes generates a clean B&W phoenix/dragon
          // as a solid filled body with a white background (passes brightness check) but
          // only 1-3 enclosed white regions inside the black fill — useless for coloring.
          if (!(await checkEnclosedRegions(binarized, name))) {
            lastResort = { buffer: binarized, contentType: 'image/png' };
            return null;
          }
          return { buffer: binarized, contentType: 'image/png' };
        }
        return result; // fallback if sharp unavailable
      }
  ...
  // Legacy fallback: HuggingFace — last tier, so (like Novita above) send the
  // raw image anyway if quality-checking/post-processing can't clean it up
  // rather than erroring. Until 2026-09-20 this tier returned straight to the
  // caller with ZERO validation — the only tier in the whole waterfall that
  // bypassed checkColoringPageQuality/checkEnclosedRegions entirely. That was
  // a real, if latent, gap: nscale's art style CAN legitimately produce
  // shaded/dark output like any other provider, and nothing was catching it
  // for this tier specifically. Routed through tryProvider() like every
  // other tier now.
  if (hfToken) {
    try {
      const result = await generateWithHuggingFace(prompt, width, height, seed, hfToken, hfModel);
      const out = await tryProvider(result, "HuggingFace");
      if (out) return out;
      console.warn("[providers] HuggingFace: quality failed and post-processing failed — sending raw image as last resort");
      return result;
    } catch (err) {
  ...
  // Absolute last resort: every tier either failed outright or was quality-rejected,
  // but at least one tier DID return a real (if imperfect) image along the way — e.g.
  // a medallion-style image with too few colorable regions. Serving that beats a hard
  // error: a kid gets something to color instead of a broken screen. Mirrors the
  // existing "send dark Novita image as last resort" precedent above.
  if (lastResort) {
    console.warn("[providers] All tiers failed/rejected — serving last rejected candidate rather than erroring");
    return lastResort;
  }
