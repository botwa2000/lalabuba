...
function _edgeArtFromGray(gray, w, h) {
  ...
  // Adaptive threshold: 50% of peak gradient — only major structural edges survive.
  // 30% was too low: kept texture noise and compression artifacts as visible edges,
  // producing scattered dots and broken lines on the coloring page.
  const THRESH = maxMag * 0.50;
  const edge = new Uint8Array(w * h);
  for (let i = 0; i < w * h; i++) if (magnitudes[i] >= THRESH) edge[i] = 1;

  // Dilate by 2 pixels (5×5 neighbourhood) for bold kid-friendly lines AND to close
  // small gaps (≤4 px) in outlines so BFS region detection can find enclosed areas.
  ...
  // Connectivity filter: remove isolated noise blobs (scattered dots from JPEG
  // compression artifacts and fine texture edges) that are too small to be part
  // of a real structural outline. BFS each connected black component; any component
  // with fewer than MIN_BLOB_PX pixels is cleared. This eliminates isolated dots
  // while preserving actual character outline strokes.
  const MIN_BLOB_PX = 60; // blobs ≤ 60px (after 5×5 dilation ≈ isolated 2-3px dots)
  ...
  // Count black pixels in output; require at least 1.5% of total to consider it usable.
  let blackCount = 0;
...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  // Tier 1: Pollinations (free, no key) — up to 2 attempts with different seeds.
  // Different seeds can produce radically different rendering styles (line art vs. shaded),
  // so a second draw is cheap and often rescues subjects that Pollinations renders dark.
  if (!skipToFreeTiers) {
    const pollinationSeeds = [seed, (seed + 31337) % 2_147_483_647];
    let pollinationsFailed = false;
    for (const pSeed of pollinationSeeds) {
      try {
        const result = await generateWithPollinations(prompt, width, height, pSeed);
        const out = await tryProvider(result, `Pollinations(seed=${pSeed})`);
        if (out) return out;
        pollinationsFailed = true;
      } catch (err) {
        if (!isQuotaOrBusy(err)) throw err;
        console.warn(`Pollinations busy (seed=${pSeed}) — trying next seed or tier`);
        pollinationsFailed = true;
      }
    }
    if (pollinationsFailed) console.warn("Pollinations both seeds failed — trying Together AI");
  } else {
