...
// Pollinations FREE tier-1 — no key needed. Uses model=turbo (SDXL Turbo) which
// produces actual coloring-page style output, unlike FLUX.1-schnell which generated
// photorealistic/engraving images. Cache HITs return in ~1-2s; cache MISSes may
// take 10-20s. Timeout is set to 12s: catches cache hits, falls through on misses.
...
function buildPollinationsUrl(prompt, width, height, seed) {
  ...
  url.searchParams.set("model",   "turbo"); // SDXL Turbo — far better coloring-page style than FLUX.1-schnell
