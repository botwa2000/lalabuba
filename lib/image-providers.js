...
// Pollinations FREE tier-1 — no key needed. Uses model=sana (NVIDIA Sana 1600M),
// the only currently listed Pollinations model. Produces clean coloring-page style
// output (white bg, black outlines, 20-30 KB JPEG on fresh generations).
// Cache HITs return in ~1-2s; cache MISSes may take 10-20s. Seed includes model
// name so switching models always busts old cached results.
// Together (402) and Cloudflare (401) both fail in ~1s, so worst-case before
// Novita is 12s + 1s + 1s = 14s. Override with POLLINATIONS_TIMEOUT_MS env var.
...
function derivePollinationsSeed(seed, prompt) {
  // Include model name in cache key so switching models (flux→sana) busts stale cache.
  // Old flux-generated images would otherwise be served for all existing prompt+seed pairs.
  const cacheKey = `sana:${prompt}`;
...
function buildPollinationsUrl(prompt, width, height, seed) {
  ...
  url.searchParams.set("model",   "sana"); // NVIDIA Sana — produces proper coloring-page line art
