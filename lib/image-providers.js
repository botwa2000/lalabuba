...
// (every novel subject+seed) goes into a slow generation queue that can take
// ~45s. Its old 50s timeout was LONGER than the client's 45s abort, so a slow
// Pollinations would hang the whole request until the user gave up — the
// waterfall never reached the faster fallback tiers. Cap it short so a cache hit
// still wins (fast + free) but a cold miss falls through quickly to the next
// provider, keeping total time comfortably under the client abort. Tune via env.
...
async function generateWithNovita(prompt, width, height, seed, apiKey) {
  ...
  const safeUrl = assertSafeImageUrl(imageUrl, ["novita.ai", "novitai.com", "amazonaws.com"]);
