...
// Combined worst case: 45s Pollinations + 2s Cloudflare + 60s Novita = 107s < 120s client budget.
const POLLINATIONS_TIMEOUT_MS = () => (Number.isFinite(_envPoll)   && _envPoll   > 0) ? _envPoll   : (dc().providers?.pollinationsTimeoutMs ?? 45_000);
// 60s: Novita typically completes in 30-60s. Must leave room after Pollinations race.
const NOVITA_TIMEOUT_MS       = () => (Number.isFinite(_envNovita) && _envNovita > 0) ? _envNovita : (dc().providers?.novitaTimeoutMs       ?? 60_000);
