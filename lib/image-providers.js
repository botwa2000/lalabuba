...
// Reduced from 50s to 25s: complex subjects regularly hit 50s, pushing total
// waterfall past the Flutter client's 90s cancel budget. 25s lets Pollinations
// succeed on fast subjects while falling through to Novita quickly on slow ones.
// Together (402) and Cloudflare (401) both fail in ~1s, so worst-case before
// Novita is 27s, leaving ~63s of Flutter budget for the paid tier to complete.
const POLLINATIONS_TIMEOUT_MS = (() => {
  const v = Number(process.env.POLLINATIONS_TIMEOUT_MS);
  return Number.isFinite(v) && v > 0 ? Math.floor(v) : 25_000;
})();
...
async function generateImage(prompt, width = 1024, height = 1024, seed, opts = {}) {
  ...
  // Pollinations misses take ~50s; Together AI credits are dead (402); Cloudflare
  ...
  // 45s + 45s alone can blow the 75s client budget before Novita even starts.
  const skipToFreeTiers = difficulty === "hard" || difficulty === "extreme";
  ...
  // Tier 2: Together AI — easy/medium only (hard/extreme go straight to Novita)
  ...
  // Tier 3: Cloudflare Workers AI — easy/medium only (hard/extreme go straight to Novita)
