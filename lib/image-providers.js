...
const POLLINATIONS_TIMEOUT_MS = () => (Number.isFinite(_envPoll)   && _envPoll   > 0) ? _envPoll   : (dc().providers?.pollinationsTimeoutMs ?? 75_000);
// Novita generation takes up to 60s; give it 90s so the paid fallback actually lands.
const NOVITA_TIMEOUT_MS       = () => (Number.isFinite(_envNovita) && _envNovita > 0) ? _envNovita : (dc().providers?.novitaTimeoutMs       ?? 90_000);
...
async function generateWithNovita(prompt, width, height, seed, apiKey) {
  ...
  // Novita generation takes 30-60s; use NOVITA_TIMEOUT_MS (90s) not the generic fetch timeout.
  let imageUrl;
  const ctrl1 = abortAfter(NOVITA_TIMEOUT_MS());
  try {
