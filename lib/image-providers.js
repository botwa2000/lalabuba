...
function _edgeArtFromGray(gray, w, h) {
  ...
  // Dilate by 1 pixel (3×3 neighbourhood) — thin, crisp lines that preserve
  // enclosed regions for BFS fill, without merging adjacent body parts into blobs.
  // 5×5 (±2px) was making lines so thick that separate features like ears, paws,
  // and facial markings merged together and enclosed regions were lost.
  ...
  // MIN_BLOB_PX reduced to 20 because 3×3 dilation produces smaller blobs per
  // edge pixel (9px vs 25px with 5×5), so the threshold must come down to match.
  const MIN_BLOB_PX = 20; // blobs ≤ 20px = isolated 1-2 pixel dots after 3×3 dilation
