...
const MODIFIER_WORDS = new Set([
  'a','an','the',
  'cute','baby','friendly','small','large','big','tiny','magical','colorful','happy',
  'fluffy','sparkling','glowing','majestic','excited','traditional','decorated','funny','playful',
  'long-neck','fire-breathing','monarch','two','three','four','five','six','seven','eight','nine','ten',
  'running','flying','swimming','jumping','sitting','sleeping','playing','hatching',
  'guarding','emerging','dancing','riding','launching','landing','holding','chasing',
]);

function subjectToNoun(subject) {
  const words = subject.toLowerCase().replace(/[^a-z0-9\s-]/g, '').split(/\s+/);
  const noun = words.find(w => w.length > 1 && !MODIFIER_WORDS.has(w));
  return noun || words[0] || 'image';
}

function generateSlug(subject, difficulty, existingSlugs = []) {
  const noun = subjectToNoun(subject);
  const base = `${noun}-${difficulty}`;
...
function backfillSlugs() {
  ...
  let changed = false;
  for (const topic of Object.keys(gallery)) {
    if (topic === '_daily') continue;
    for (const diff of DIFFICULTIES) {
      const entries = gallery[topic]?.[diff];
      if (!Array.isArray(entries)) continue;
      const seen = [];
      for (const entry of entries) {
        if (!entry.slug) {
          entry.slug = generateSlug(entry.subject || topic, diff, seen);
          changed = true;
        }
  ...
  if (changed) saveGallery(gallery);
}
...
async function addToGallery(topic, difficulty, subject, url) {
  ...
  const existingSlugs = gallery[topic][difficulty].map(e => e.slug).filter(Boolean);
  const slug  = generateSlug(subject, difficulty, existingSlugs);
  const entry = { id: `${topic}-${difficulty}-${Date.now()}`, subject, url, slug, date: todayKey() };
...
module.exports = {
  ...
  generateSlug, subjectToNoun, backfillSlugs,
};
