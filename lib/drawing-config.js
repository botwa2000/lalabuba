...
const DEFAULTS = {
  ...
  // skipFreeTiers removed from all difficulties.
  // Rationale: when Novita (paid) hits its daily cap, skipping Pollinations left
  // hard/extreme with no fallback → 500. Pollinations cold-miss ≈ 50s; Cloudflare
  // 401-fails fast; Novita ≈ 10-15s. Worst-case chain: 50 + 1 + 15 = 66s < 80s
  // client budget. Better a slower draw than a 500 error.
  difficulties: {
    easy:    { minArea: 2000, maxRegions: 10, colorCount:  6, resolution: 1024 },
    medium:  { minArea:  800, maxRegions: 18, colorCount:  8, resolution: 1024 },
    hard:    { minArea:  300, maxRegions: 30, colorCount: 12, resolution: 1024 },
    extreme: { minArea:   80, maxRegions: 48, colorCount: 99, resolution: 1024 },
  },
  ...
  // ── 8. GENERATION ─────────────────────────────────────────────────────────────
  // clientTimeoutMs: hard abort on the /api/generate-image fetch.
  //   web=75s; Flutter=150s (raised from 80s so Novita gets its full 80s budget
  //   even when Pollinations burns 45s before failing). SERVER CLIENT_BUDGET_MS
  //   must be kept at clientTimeoutMs - 10000.
